                ...
                <div class="col-md-12 ">
                    ...
                    <div class="card ml-5" style="border-radius: 5px">
                        <div class="card-body " style="background-color: #fafafa;border-radius: 5px" dir="rtl">
                            ...
                            <!--Section heading-->
                            <!--Grid column-->
                            <form action="{{route('profile.twofactor.start.post')}}" method="POST" style="font-size: .8rem">
                            @csrf
                                        ...
                                        <div class="form-group text-right">
                                            ...
                                            <select name="type" id="2fa_type" class="form-control font-weight-bold @error('token') is-invalid @enderror">
                                                @foreach(config('twofactor.types') as $key => $name)
                                                    <option class="font-weight-bold" value="{{ $key }}" {{ old('type') == $key || auth('web')->user()->hasTwoFactor($key)  ? 'selected' : '' }}>{{ $name }}</option>
                                                @endforeach
                                            </select>
                                            @error('type')
                                            <span class="invalid-feedback">
